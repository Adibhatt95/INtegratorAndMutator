...
namespace IntegratorANDMutator
{
    ...
    {//this is the heavily modified code by me, Aditya. see comments in the code for clarity. If any more questions, then contact me on the email thread.
        ...
        {
            ArgsCardClass = args[1].ToString();// "hunter";// 
            ...
            string folderName = args[0].ToString();//"2d7d2018-11-26-56-AM";//"Level-0";//"Level-0";//"Level-3"; //"Level-4";//"Warlock4-0";//"D:\\WarlockStarter\\evoevoHunter-0";////
            ...
            while (true)
            {
                ...
                {
                    Console.WriteLine("for  evoluation code found " + folderName + " checking if number of items =  40,000");
                    Thread.Sleep(1000);
        ...
        {//mutate bottom 90% decks
            ...
            int[] topTenIDs = new int[top10Decks.Count];
            Console.WriteLine("ToptenIDs length=" + topTenIDs.Length);
            int k = 0;
            Console.WriteLine("top ten decks=");
            foreach (int topTenID in top10Decks.Keys)
            {
                topTenIDs[k] = topTenID;
                k++;
                Console.Write(" " + topTenID + ",");
            }
            int jTrackerMax9 = 0;
            int currTopTenID = 0;
            for (int i = 0; i < nDecks.Count; i++)
            {
                ...
                {
                    Console.WriteLine("Mutating Deck=" + i + ": ...replacing with mutated version of deck=" + topTenIDs[currTopTenID]);
                    bool mutateorNot = true;
                    int j = 0;
                    CreateAndMutate createandMutate = new CreateAndMutate();
                    Console.WriteLine("the deck "+ topTenIDs[currTopTenID]+" after shuffle:");
                    List<Card> topTenDeckCurr = nDecks[topTenIDs[currTopTenID]];
                    
                    createandMutate.print(topTenDeckCurr);
                    Random rand = new Random();
                    int x = rand.Next(0,30);
                    Thread.Sleep(1000);
                    int cardToSwap = cardname[topTenDeckCurr[x].Name];
                    nDecks[i] = createandMutate.mutateSpecificCard(topTenDeckCurr, allCards, cardname, cardToSwap);
                    j++;
                    while (mutateorNot && j < 30)
                    {
                        ...
                        if (r.Next(0, 2) == 0)
                        {
                            x = rand.Next(0, 30);
                            Thread.Sleep(1000);
                            cardToSwap = cardname[nDecks[i][x].Name];
            ...
            Console.WriteLine("currtopTenID=(should be 9)=" + currTopTenID);
            return nDecks;
        }
